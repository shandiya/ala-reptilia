...
### Seasonal count differences

```{r}

reptilia_wider <- reptilia_tidy %>%
  filter(!is.na(month)) %>%
  group_by(family, month) %>%
  summarise(occ = n()) %>% 
  pivot_wider(names_from = month, values_from = occ) %>% 
  mutate_if(is.numeric, replace_na, 0) %>% 
  relocate(Jul, .before = Aug) %>% 
  mutate(total = sum(c_across(Jan:Dec)),
         max = max(c_across(Jan:Dec))) %>% 
  filter(total > 100)

max_fam <- reptilia_wider$max
  
reptilia_radar <- reptilia_wider %>% 
  select(family:Dec)


ggradar(
  reptilia_radar[1, ],
  grid.mid = (max_fam[1]) / 2,
  grid.max = max_fam[1],
  values.radar = NA,
  group.line.width = 1,
  group.point.size = 0,
  ...
  plot.title = reptilia_radar$family[1]
 )
